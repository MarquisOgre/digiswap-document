...
The **Injector** or **Owner** can call this function to inject a specific _lotteryId_ with a specified amount of _DIGIS_.
...
function drawFinalNumberAndMakeLotteryClaimable(uint256 _lotteryId, bool _autoInjection)
        ...
        nonReentrant
    {
        ...
        // Calculate prizes in DIGIS for each bracket by starting from the highest one
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                // A. No DIGIS to distribute, they are added to the amount to withdraw to treasury address
            } else {
        ...
        // Transfer DIGIS to treasury address
...
```typescript
   function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(cakeToken), "Cannot be DIGIS token");
...
In the case of tokens other than DIGIS mistakenly being sent to the lottery contract, this function is used to recover them and is only callable by the **Owner**
